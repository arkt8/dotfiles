...
syn keyword sqlKeyword ABORT ACTION AFTER ALL ALTER ANALYZE AND AS ASC ATTACH
syn keyword sqlKeyword AUTOINCREMENT BEFORE BEGIN BETWEEN BY CASCADE CASE CAST CHECK COLLATE COLUMN
syn keyword sqlKeyword COMMIT CONFLICT CONSTRAINT CROSS CURRENT_DATE CURRENT_TIME
syn keyword sqlKeyword CURRENT_TIMESTAMP DATABASE DEFAULT DEFERRABLE DEFERRED DESC DETACH
syn keyword sqlKeyword DISTINCT EACH ELSE END ESCAPE EXCEPT EXCLUSIVE EXISTS EXPLAIN FAIL FOR
...
syn keyword sqlKeyword RESTRICT RIGHT ROLLBACK ROW SAVEPOINT SET TABLE TEMP TEMPORARY
syn keyword sqlKeyword THEN TO TRANSACTION TRIGGER UNION UNIQUE USING VACUUM VALUES VIEW
syn keyword sqlKeyword VIRTUAL WHEN WHERE WITH WITHOUT

syn keyword sqlVerb    CREATE SELECT DELETE UPDATE DROP ADD REPLACE
...
" Identifiers for binding va:lues to arguments, as defined in http://www.sqlite.org/c3ref/bind_blob.html
syn match sqlNumber '?' 
...
if version >= 508 || !exists("did_sql_syn_inits")
	...
	HiLink sqlVerb          Statement
    HiLink sqlKeyword       Operator
